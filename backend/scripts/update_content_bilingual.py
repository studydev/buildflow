...
async def update_content_bilingual():
    """Update content items with bilingual fields from analysis results."""

    # Read .env.local and get the LAST occurrence of COSMOS_CONNECTION_STRING (Azure, not emulator)
    import re
    ...
    if not connection_string:
        print("❌ COSMOS_CONNECTION_STRING not found in .env.local")
        return

    # Skip localhost emulator, use Azure connection
    if "localhost" in connection_string:
        print("⚠️  Found localhost emulator connection, looking for Azure connection...")
        # Re-read and get Azure connection (not localhost)
        with open(env_file) as f:
            for line in f:
                line = line.strip()
    ...
    if "localhost" in connection_string:
        print("❌ Only localhost connection found. Need Azure Cosmos DB connection.")
        return
    ...
    async with CosmosClient(cosmos_endpoint, cosmos_key) as client:
        ...
        analysis_requests = []
        async for item in analysis_container.query_items(
            query=query,
            parameters=None
        ):
            analysis_requests.append(item)
        ...
        for ar in analysis_requests:
            ...
            for content_id in content_ids:
                try:
                    # Read content item
                    content = await content_container.read_item(
                        item=content_id,
                        partition_key=content_id  # partition key is /id
                    )
                    ...
                    await content_container.upsert_item(content)
...
if __name__ == "__main__":
    asyncio.run(update_content_bilingual())
